...
3. **video** — narrates the script (ElevenLabs), times karaoke-style captions to the real audio, and composites a vertical 9:16 draft. Visuals are relevance-checked stock video clips (Pexels) *or* icon animations (Iconify) — never mixed within one video. Stock video is only used if every line finds a genuinely relevant clip; if even one doesn't, the whole video uses icons instead
...
- **Video/icon matches aren't always perfect.** Every stock video candidate is vision-checked against the line before use, and icon selection is best-effort semantic matching (via Iconify) — but neither is guaranteed on-topic for every line, which is exactly what the review step is for. A video is all-stock-video or all-icons, never mixed — if even one line doesn't find a relevant clip, the whole video falls back to icons. Because of that, stock video coverage varies a lot by domain: topics with thin real stock-footage coverage (e.g. spaceflight) will end up all-icons most of the time, and that's expected, not a bug.
