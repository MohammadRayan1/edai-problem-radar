...
3. **video** — narrates the script (ElevenLabs), times karaoke-style captions to the real audio, and composites a vertical 9:16 draft. Visuals are relevance-checked real imagery — stock video clips *and* stock photos (both Pexels, photos get a slow Ken Burns zoom) — *or* icon animations (Iconify) — never mixed with icons within one video, though video and photo can mix with each other freely. Real imagery is only used if every line finds a genuinely relevant clip or photo; if even one doesn't, the whole video uses icons instead
...
| `PEXELS_API_KEY` | No | [pexels.com/api](https://www.pexels.com/api/) | Stock video clips *and* stock photos for line visuals (same key, two endpoints). Free, generous rate limits. If unset, every line just uses an icon (no error, no missing feature — the pipeline was icon-only before this existed) |
...
- **Video/icon matches aren't always perfect.** Every stock candidate (video or photo) is vision-checked against the line before use, and icon selection is best-effort semantic matching (via Iconify) — but neither is guaranteed on-topic for every line, which is exactly what the review step is for. A video is all-real-imagery or all-icons, never mixed with icons — if even one line doesn't find a relevant clip or photo, the whole video falls back to icons. To maximize real-imagery coverage, each line checks multiple search candidates (video first, then photo) round by round rather than giving up after one failed match, which meaningfully improves coverage on thin-footage topics like spaceflight — but coverage still varies by domain, and all-icons remains expected for some topics, not a bug.
